...
(def input (slurp "src/aoc/day1.input"))
...
(defn get-floor [parens]
  (reduce (fn up-or-down [floor paren]
            (if (= paren \()
              (inc floor)
              (dec floor)))
          0
...
(defn get-position [parens]
  (reduce (fn floor-and-position [[floor position] paren]
            (let [new-position (inc position)
                  new-floor (if (= paren \() (inc floor) (dec floor))]
            (if (basement? new-floor)
              (reduced new-position)
              [new-floor new-position])))
          [0 0]
          parens)
  )
