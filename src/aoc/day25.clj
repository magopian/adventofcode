...
(defn get-nth
  "Given a row and a column, return the 'index' of the cell."
  [row col]
  (->> ; The row that has the beginning of the diagonal where the cell lies is
       ; one less than row + col. Eg. the row that has the beginning of the
       ; diagonal that has the cell "19" (which is at [3 4]) is (3 + 4) - 1 = 6
       (dec (+ row col))
       ; Each beginning of a diagonal holds the number of previous diagonals'
       ; cells. So the 5th diagonal (starting on row 5) has 4 + 3 + 2 + 1 = 10
       (range 0)
       (apply +)
       ; You then only need to add the cols to get to the cell you want.
       (+ col)))

(assert (= (get-nth 1 1) 1))
(assert (= (get-nth 1 5) 15))
(assert (= (get-nth 1 6) 21))
(assert (= (get-nth 5 1) 11))
(assert (= (get-nth 3 4) 19))

(defn get-next-code [code]
  (rem (* code 252533) 33554393))

(assert (= (get-next-code 20151125) 31916031))
...
(defn part1 []
  (println
    "The code is:"
    (last (take (get-nth row column) (iterate get-next-code 20151125)))
    ))
...
; All done \o/ That was really awesome and so much fun!
